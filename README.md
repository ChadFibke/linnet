...
data, so if you would need to save the graph to file and read it in if
you would like to reproduce the results you achieved on a given day.
...
categorize all other recombinant into the “Recombinants” category.
...
your data. This functions works with any quantity (relative frequency,
...
their descendants, are pruned, and reattached to the root lineage. This
is to avoid propagating the recombinant values twice. The code will then
conduct a cumulative sum of the lineage values from the leaves/bottom of
the graph to the root/top. This cumulative sum uses a breadth first
search to sum the values of all the direct children of a lineage to the
lineage. This process is repeated until a threshold is met. Once met,
the cumulative sum is reset to zero and traverses up the tree as above,
but ignores the lineages which has met the threshold. The code then
returns all lineages which have met the threshold. The most recent
common ancestor is also returned if there are any remaining lineage
values not contributing to the above identify lineages. This serves as
an all others bin.

![](man/figures/step2.png)

The identified lineages can then be used in the categorize function to
group all supporting sub-lineages into their predominant parental
groups. In turn, this can simplify noisy plots to more predominant
lineages.
